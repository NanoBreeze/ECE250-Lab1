...
#ifndef CYCLIC_LIST_TESTER_H
#define CYCLIC_LIST_TESTER_H
...
class Cyclic_list_tester :public Tester< Cyclic_list<Type> > {
	using Tester< Cyclic_list<Type> >::object;
	using Tester< Cyclic_list<Type> >::command;

public:
	Cyclic_list_tester(Cyclic_list<Type> *obj = nullptr) :Tester< Cyclic_list<Type> >(obj) {
		// empty
	}
...
*   head            head            the head is not 'nullptr' and sends the next N instructions to Single_node_tester
*   head0           head            the head is null
*   tail            tail            the head is not 'nullptr' and sends the next N instructions to Single_node_tester
*   tail0           tail            the head is null
*   count n b       count           the number of occurances of n is m
*
*  Mutators
*
...
template <typename Type>
void Cyclic_list_tester<Type>::process() {
	if (command == "new") {
		object = new Cyclic_list<Type>();
		...
		Single_node<Type> *actual_head = object->head();

		if (actual_head == nullptr) {
			std::cout << "Failure in head(): expecting a non-null head pointer" << std::endl;
		}
		else {
			std::cout << "Okay" << std::endl;

			Single_node_tester<Type> tester(actual_head);
		...
		if (object->head() == nullptr) {
			...
			std::cout << "Failure in head(): expecting a null head pointer" << std::endl;
		}
	}
		...
		Single_node<Type> *actual_tail = object->tail();

		if (actual_tail == nullptr) {
			std::cout << "Failure in tail(): expecting a non-null tail pointer" << std::endl;
		}
		else {
			std::cout << "Okay" << std::endl;

			Single_node_tester<Type> tester(actual_tail);

			tester.run();
		}
	}
	else if (command == "tail0") {
		// check that the tail pointer is null

		if (object->tail() == nullptr) {
			std::cout << "Okay" << std::endl;
		}
		else {
			std::cout << "Failure in tail(): expecting a null tail pointer" << std::endl;
		}
	}
		...
		Cyclic_list<Type> *new_object = new Cyclic_list<Type>();

		*new_object = *object;

		std::cout << "Okay" << std::endl;

		Cyclic_list_tester<Type> tester(new_object);
